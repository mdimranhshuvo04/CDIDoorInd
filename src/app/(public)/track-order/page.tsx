...
export default function TrackOrderPage() {
    ...
    const steps = [
        { label: t('store.track_order.placed') || 'Order Placed', icon: Clock },
        { label: t('store.track_order.confirmed') || 'Confirmed', icon: CheckCircle2 },
        { label: t('store.track_order.processing') || 'Processing', icon: Package },
        { label: t('store.track_order.released') || 'Released for Delivery', icon: Truck },
        { label: t('store.track_order.delivered') || 'Delivered', icon: MapPin },
    ];
        ...
        <div className="min-h-screen bg-muted/30 py-20 px-4">
            <div className="max-w-3xl mx-auto space-y-8">
                ...
                <div className="text-center space-y-4">
                    ...
                    <h1 className="text-4xl font-black tracking-tighter sm:text-5xl">{t('store.track_order.title') || 'Track Your Order'}</h1>
                    <p className="text-muted-foreground max-w-md mx-auto">
                        {t('store.track_order.subtitle') || 'Enter your Order ID to see the real-time status of your shipment.'}
                    </p>
                </div>
                    ...
                    <CardContent className="p-8">
                        <form onSubmit={handleTrack} className="flex flex-col sm:flex-row gap-4">
                            <div className="relative flex-1">
                                ...
                                <Input 
                                    placeholder={t('store.track_order.input_placeholder') || 'Enter Order ID (e.g. #ORD12345)'}
                            ...
                            >
                                {loading ? (t('store.track_order.searching') || 'Searching...') : (t('store.track_order.track_now') || 'Track Now')}
                            </Button>
                        </form>
                    </CardContent>
                </Card>
                    ...
                    <Card className="border-none shadow-2xl rounded-3xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <CardHeader className="bg-primary text-primary-foreground p-8">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                <div>
                                    <p className="text-primary-foreground/70 text-sm font-bold uppercase tracking-widest">{t('store.track_order.order_id_label') || 'Order ID'}</p>
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    {isCurrent && (
                                                        <p className="text-[10px] text-primary font-black uppercase tracking-tighter animate-pulse">
                                                            {t('store.track_order.currently_here') || 'Currently Here'}
                                                        </p>
                                                    )}
                                                </div>
                                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t">
                                <div className="space-y-4">
                                    <h3 className="font-bold text-lg flex items-center gap-2">
                                        <Package className="h-5 w-5 text-primary" /> {t('store.track_order.delivery_details') || 'Delivery Details'}
                                    </h3>
                                    <div className="space-y-3 bg-muted/50 p-6 rounded-2xl">
                                        <div>
                                            <p className="text-xs text-muted-foreground font-bold uppercase">{t('store.track_order.recipient_name') || 'Recipient Name'}</p>
                                            <p className="font-bold">{order.shippingDetails?.name || 'N/A'}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-muted-foreground font-bold uppercase">{t('store.track_order.shipping_address') || 'Shipping Address'}</p>
                                            <p className="font-medium text-sm leading-relaxed">{order.shippingDetails?.address || 'N/A'}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-muted-foreground font-bold uppercase">{t('store.track_order.phone_number') || 'Phone Number'}</p>
                                ...
                                <div className="space-y-4">
                                    <h3 className="font-bold text-lg flex items-center gap-2">
                                        <Clock className="h-5 w-5 text-primary" /> {t('store.track_order.order_timeline') || 'Order Timeline'}
                                    </h3>
                                    <div className="space-y-4 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-muted">
                                        <div className="relative pl-8">
                                            ...
                                            <p className="text-sm font-bold">{t('store.track_order.order_received') || 'Order Received'}</p>
                                        ...
                                        <div className="relative pl-8">
                                            ...
                                            <p className={`text-sm font-bold ${currentStep >= 1 ? 'opacity-100' : 'opacity-40'}`}>{t('store.track_order.verification') || 'Verification & Confirmation'}</p>
                                        </div>
                                        <div className="relative pl-8">
                                            ...
                                            <p className={`text-sm font-bold ${currentStep >= 3 ? 'opacity-100' : 'opacity-40'}`}>{t('store.track_order.shipped_on_way') || 'Shipped & On the Way'}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                {!order && !loading && orderId && (
                    <div className="bg-red-500/10 border-2 border-red-500/20 p-8 rounded-3xl text-center space-y-3">
                        ...
                        <h3 className="text-xl font-bold text-red-500">{t('store.track_order.not_found') || 'Order ID Not Found'}</h3>
                        <p className="text-sm text-red-600/70">
                            {t('store.track_order.not_found_desc') || "We couldn't find any order with the ID you provided. Please double-check your receipt or email confirmation."}
                        </p>
                    </div>
                )}
            </div>
