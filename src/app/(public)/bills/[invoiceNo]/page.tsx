...
import { Loader2, Download, Landmark, Calendar, Phone, MapPin, CheckCircle, AlertTriangle, CreditCard } from 'lucide-react';
...
export default function PublicBillPage(props: { params: Promise<{ invoiceNo: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch Bill Details
        const billRes = await fetch(`/api/public/bills/${invoiceNo}`);
        if (!billRes.ok) {
          if (billRes.status === 404) throw new Error('Invoice not found');
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-muted/30">
        ...
        <p className="mt-4 text-muted-foreground font-semibold">Loading invoice details...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-muted/20 py-8 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-4xl mx-auto space-y-6">
          ...
          <CardContent className="p-6 sm:p-10 space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b pb-6">
              ...
              <div className="text-left md:text-right">
                <Badge 
                  ...
                  className={`text-xs px-3 py-1 font-bold ${bill.status === 'Paid' ? 'bg-green-600 hover:bg-green-600 text-white' : ''}`}
                >
                  {bill.status}
                </Badge>
                <div className="text-sm font-semibold text-foreground mt-2">Invoice: {bill.invoiceNo}</div>
                <div className="text-xs text-muted-foreground mt-1">Date: {format(new Date(bill.date), 'dd MMM yyyy')}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
              <div className="space-y-2 p-4 rounded-xl bg-muted/40 border">
                <h3 className="font-bold text-muted-foreground text-[11px] uppercase tracking-wider">Billing To</h3>
                <div className="font-bold text-base text-foreground">{bill.clientName}</div>
                <div className="flex items-center gap-1.5 text-muted-foreground text-xs mt-1">
                  <Phone className="w-3.5 h-3.5" /> {bill.clientPhone}
                </div>
                <div className="flex items-start gap-1.5 text-muted-foreground text-xs mt-1">
                  <MapPin className="w-3.5 h-3.5 mt-0.5" /> {bill.clientAddress}
                </div>
              </div>
              ...
              {bill.showroom ? (
                <div className="space-y-2 p-4 rounded-xl bg-muted/40 border">
                  ...
                  <div className="font-bold text-base text-foreground">{bill.showroom.name}</div>
                  {bill.showroom.phone && (
                    <div className="flex items-center gap-1.5 text-muted-foreground text-xs mt-1">
                      <Phone className="w-3.5 h-3.5" /> {bill.showroom.phone}
                    </div>
                  )}
                  {bill.showroom.address && (
                    <div className="flex items-start gap-1.5 text-muted-foreground text-xs mt-1">
                      <MapPin className="w-3.5 h-3.5 mt-0.5" /> {bill.showroom.address}
                    </div>
                  )}
                </div>
              ) : (
            ...
            <div className="flex justify-end">
              <div className="w-full sm:w-80 space-y-2 border-t pt-4 text-sm text-muted-foreground">
                <div className="flex justify-between">
                  <span>Subtotal</span>
                  <span className="font-semibold text-foreground">৳{bill.subtotal.toLocaleString('en-BD')}</span>
                </div>
                {bill.deliveryCharge > 0 && (
                  <div className="flex justify-between">
                    <span>Delivery Charge</span>
                    <span className="font-semibold text-foreground">+ ৳{bill.deliveryCharge.toLocaleString('en-BD')}</span>
                  </div>
                )}
                {bill.serviceFee > 0 && (
                  <div className="flex justify-between">
                    <span>Service Fee</span>
                    <span className="font-semibold text-foreground">+ ৳{bill.serviceFee.toLocaleString('en-BD')}</span>
                  </div>
                )}
                {bill.discount > 0 && (
                  <div className="flex justify-between text-green-600 font-medium">
                    <span>Discount {bill.discountType === 'percentage' ? `(${bill.discountValue}%)` : ''}</span>
                    <span>- ৳{bill.discount.toLocaleString('en-BD')}</span>
                  </div>
                )}
                <div className="flex justify-between border-t pt-2 text-base font-bold text-foreground">
                  <span>Total</span>
                  <span>৳{bill.total.toLocaleString('en-BD')}</span>
                </div>
                {bill.prevDue > 0 && (
                  <div className="flex justify-between text-rose-600">
                    <span>Previous Due</span>
                    <span>+ ৳{bill.prevDue.toLocaleString('en-BD')}</span>
                  </div>
                )}
                <div className="flex justify-between border-t-2 border-double pt-2 text-lg font-black text-foreground">
                  <span>Grand Total</span>
                  <span>৳{bill.gTotal.toLocaleString('en-BD')}</span>
                </div>
                <div className="flex justify-between text-green-600 font-bold">
                  <span>Amount Paid</span>
                  <span>৳{bill.cashIn.toLocaleString('en-BD')}</span>
                </div>
                <div className="flex justify-between text-rose-600 font-black border-t border-dashed pt-1.5 text-xl">
                  <span>Current Due</span>
                  <span>৳{bill.currentBillDue.toLocaleString('en-BD')}</span>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        {/* Payment Instructions Panel (Only shown if Status is Due) */}
        {bill.status === 'Due' && (manualPay?.bkash?.active || manualPay?.nagad?.active || manualPay?.rocket?.active || isBankConfigured || manualPay?.instructions) && (
          <Card className="border-2 border-primary/20 bg-primary/5 rounded-2xl shadow-sm overflow-hidden">
            <CardContent className="p-6 sm:p-8 space-y-6">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">Please use one of the payment options below to pay your remaining due</p>
              </div>
